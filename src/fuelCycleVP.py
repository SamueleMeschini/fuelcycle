...
#from vacuumPumpSystem import VacuumPumpSystem
from cryopumpSystem import CryopumpSystem
...
#max_pump_capacity measured in standard m^3ù
#max_pump_throughput = 5.6e-5 computed considering 1Pa and 573K at divertor 
max_pump_capacity = 0.040 #kg --> computed from volume = 150 stdL
...
VP = CryopumpSystem("VP", max_pump_capacity, 5.6e-5, 300, 0)
...
simulation = Simulate(dt=10, dt_max = 100, final_time=final_time, I_reserve=I_reserve, component_map=component_map, max_simulations=100, TBRr_accuraty = 1e-1)
t, y = simulation.run()
